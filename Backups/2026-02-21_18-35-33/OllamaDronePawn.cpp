...
void AOllamaDronePawn::SnapAndSend()
{
	...
	// FString TopDownBase64;
	// if (bIncludeTopDownImage && TopDownCapture)
	// {
	// 	TopDownCapture->CaptureScene();
	// 	TopDownBase64 = RenderTargetToBase64(TopDownRenderTarget);
	// }
	...
	// Build perception context instead of raw actor dump
                                     ...
                                     const FString& ContextText)
{
	...
	// -- LMStudio (local) -- commented out for Gemini test
	// Request->SetURL(TEXT("http://localhost:1234/v1/chat/completions"));

	// -- Gemini Pro --
	// Get your API key from environment
	FString GeminiKey = FPlatformMisc::GetEnvironmentVariable(TEXT("GEMINI_API_KEY"));
	
	if (GeminiKey.IsEmpty())
	{
		UE_LOG(LogTemp, Error, TEXT("Drone: GEMINI_API_KEY not set in environment"));
		GEngine->AddOnScreenDebugMessage(-1, 10.f, FColor::Red,
			TEXT("Drone: GEMINI_API_KEY missing!"));
		return;
	}
                                          ...
                                          bool bWasSuccessful)
{
	...
	if (FJsonSerializer::Deserialize(Reader, JsonObject))
	{
		...
		if (Candidates.Num() > 0)
		{
			...
			UE_LOG(LogTemp, Warning, TEXT("Drone Vision: %s"), *Content);
			GEngine->AddOnScreenDebugMessage(-1, 20.f, FColor::Cyan,
